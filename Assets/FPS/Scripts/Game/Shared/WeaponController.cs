...
namespace Unity.FPS.Game
{
    ...
    public class WeaponController : MonoBehaviour
    {
        ...
        void Awake()
        {
            //tu debugnac czy sie nazywa player - to sie nazywa weapon blaster albo weapon machinegun
        ...
        public bool HandleShootInputs(bool inputDown, bool inputHeld, bool inputUp, bool playerShoot)
        {
            if (!playerShoot && !adjustmentMode && gameObject.tag == "Enemy_EyeLazers")
            {
                if (easyMode)
                {
            ...
            switch (ShootType)
            {
                ...
                case WeaponShootType.Charge:
                    ...
                    {
                        return TryReleaseCharge(playerShoot);
                    }
        ...
        bool TryShoot(bool playerShoot)
        {
            if (m_CurrentAmmo >= 1f
                && m_LastTimeShot + DelayBetweenShots < Time.time)
            {
                HandleShoot(playerShoot);
                m_CurrentAmmo -= 1f;
        ...
        bool TryReleaseCharge(bool playerShoot)
        {
            if (IsCharging)
            {
                HandleShoot(playerShoot);
        ...
        void HandleShoot(bool playerShoot)
        {
            if (adjustmentMode && playerShoot && m_DataManager != null)
            {
                m_DataManager.BulletShooted++;
            }
